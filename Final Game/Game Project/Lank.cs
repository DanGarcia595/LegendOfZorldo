...
namespace Game_Project
{
    class Lank:Character
    {
        ...
        public void Fight(Character b) //Lank.Fight(b) must use to decide enemy to fight
        {
            ...
            if (MainGame.CurrentRoom.CHAR.Find(en => en.Name == b.Name).Health <= 0)
                Temp.character1 = MainGame.CurrentRoom.CHAR.Find(en => en.Name == b.Name);
            else
                Interface.print(b.Name + " has " + b.Health + " Health");
            Interface.Refresh();
        ...
        public new void Die()
        {
            ...
            Interface.DrawInput();
            bool OK = MainGame.GetYN("You have died....Would you like to restart?", "Enter YES or NO.");
            if (!OK)
                Environment.Exit(0);
            return;
