...
namespace Game_Project
{
    ...
    class MainGame
    {
      /*  private const int MF_BYCOMMAND = 0x00000000;
        ...
        static public List<Chest> ChestL = new List<Chest>();
        static public List<Command> CommandL = new List<Command>();
		static public List<Room> RoomL = new List<Room>();
        static public System.Media.SoundPlayer sp = new System.Media.SoundPlayer();
        ...
        static void Main(string[] args)
        {
          ...
          //  DeleteMenu(GetSystemMenu(GetConsoleWindow(), false), SC_MINIMIZE, MF_BYCOMMAND);
            ...
            Interface.StartDraw();
        START:
            sp = new System.Media.SoundPlayer(@"LoZsong.wav");
            sp.PlayLooping();
            Console.Clear();
            MainGame.ItemL = Items();
            MainGame.ChestL = Chests();
            MainGame.CommandL = Commands();
            MainGame.RoomL = Room.CreateRooms();
            MainGame.Lank = new Lank();
           ...
           //            Interface.print("Chest L is open!");   I LOVE LISTS!
                ...
                if (Lank.Health == 400)
                {
                    ...
                    Interface.print("Game Developed By:");
                    Interface.print("Bobby Davis, Daniel Garcia, Andrew Mele, and Richard Muri");
                    Console.CursorVisible = false;
                    Console.ReadKey();
                    Console.CursorVisible = true;
                    bool OK = GetYN("Would you like to play again?", "Please enter YES or NO.");
                    if(!OK)
                        Environment.Exit(0);
                    goto START;
              ...
              if (Temp.character1.Name != "")
              {
                  Temp.character1.Die();
                  CurrentRoom.CHAR.Remove(CurrentRoom.CHAR.Find(tmp => tmp == Temp.character1));
                  if (Temp.character1.Name == "Ganondalf")
                      Interface.DrawChest(CurrentRoom.CHEST.ElementAt(0));
                  Temp.character1.Name = "";
                  Interface.Refresh();
              }
               foreach (Character p in CurrentRoom.POTS)
                {
        ...
        {                               //when referring to an item use its id-1
            ...
            Item ArrowsD = new Item("ARROWS", "These finely cut arrows will surely pierce anything stupid enough to get\nin your way.\n", "Utility", 1, 021);

            Item[] Itemz = { WoodenSword, BrokenShield, KokiriSword, KokiriShield, DinsEdge, NayrusTears, Bow, Boomerang, Bombs, Arrows, PegasusBoots, Hookshot, SmallKey, BossKey, DungeonMap, Compass, HeartContainer, Heart, Fairy, rupee, ArrowsD };
            //I = Itemz.ToList();
            I.AddRange(Itemz);
            return I;
        }
        ...
        {                                //when referring to chest refer to it by its id-50
            ...
            List<Item> I = Items();
            Chest kokirisword = new Chest(I.ElementAt(2), 22, 13, 050);
            Chest kokirishield = new Chest(I.ElementAt(3), 22, 14, 051);
            Chest dinsedge = new Chest(I.ElementAt(4), 27, 0, 052);
            Chest nayrustears = new Chest(I.ElementAt(5), 27, 17, 053);
            Chest bow = new Chest(I.ElementAt(6), 27, 14, 054);
            Chest boomerang = new Chest(I.ElementAt(7), 21, 4, 055);
            Chest bombs = new Chest(I.ElementAt(8), 0, 0, 056);
            Chest pegasusboots = new Chest(I.ElementAt(10), 2, 16, 057);
            Chest hookshot = new Chest(I.ElementAt(11), 0, 0, 058);
            Chest smallkey = new Chest(I.ElementAt(12), 0, 0, 059);
            Chest bosskey = new Chest(I.ElementAt(13), 4, 3, 060);
            Chest dungeonmap = new Chest(I.ElementAt(14), 7, 8, 061);
            Chest compass = new Chest(I.ElementAt(15), 20, 1, 062);
            Chest heartcontainer = new Chest(I.ElementAt(16), 13, 2, 063, true);
            ...
            Chest rupee20 = new Chest(r20, 27, 17, 066);
            Item r50 = new Item("RUPEE", "hjjhghjg", "hhjgjh", 50, 676776);
            Chest rupee50 = new Chest(r50, 4, 6, 067);
        ...
        public static List<Character> Characters()
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                enemyItem[i] = I.ElementAt(rand.Next(18, 22) - ITEMOFFSET);
            }

            List<Character> E = new List<Character>();
            Character Skulltula = new Character("Skulltula", "These guys are giant spiders and fall on you at the most inopportune times", 102, 100, 10, 50, 10, enemyItem[1], 0, 0, 0, false, 'a');
            Character Lizalfos = new Character("Lizalfos", "Dude....Giant Lizard People!", 103, 200, 40, 60, 30, enemyItem[2], 0, 2, 0, false,'φ');
            Character Keese = new Character("Keese", "These bat-like attrocities will surely get on your nerves", 104, 100, 20, 50, 10, enemyItem[3], 0, 0, 0, false, 'ß');
            Character Stalfos = new Character("Stalfos", "Some living skeleton that like to poke you with their sword", 105, 200, 10, 75, 30, enemyItem[4], 0, 0, 0, false, '♣');
            Character Likelike = new Character("Like-Like", "Amorphous blobs that surround and digest you", 106, 200, 10, 50, 50, enemyItem[5], 0, 0, 0, false, '☼');            
            Character Navi = new Character("Navi", "Hey! Listen!", 104, 100, 20, 20, 10, enemyItem[3], 10, 10, 0, false, '*');
 	        Character Ganondalf = new Character("Ganondalf", "A once nice old wizard turned lord of evil who stole the Princess Zorldo", 107, 1000, 50, 100, 50, enemyItem[6], 13, 8, 316, false, 'δ');
            Character[] enemies = { Skulltula, Lizalfos, Keese, Stalfos, Likelike, Ganondalf,Navi };
            E = enemies.ToList();
            //E.AddRange(enemies);
            return E;
        }
        public static List<Character> Ganon()
        {
            List<Character> c = new List<Character>();
            c.Add(Characters().ElementAt(5));
            return c;
        }
        public static List<Character> Pots()
        {
            ...
            Character pot5 = new Character("pot 5", "It's a pot", 120, 1, 0, 0, 0, enemyItem[5], 12, 12, 0, false, '₧');
            Character pot6 = new Character("pot 6", "It's a pot", 120, 1, 0, 0, 0, enemyItem[6], 13, 14, 0, false, '₧');
            Character pot7 = new Character("pot 7", "It's a pot", 120, 1, 0, 0, 0, enemyItem[7], 10, 3, 0, false, '₧');
            Character pot8 = new Character("pot 8", "It's a pot", 120, 1, 0, 0, 0, enemyItem[8], 4, 6, 0, false, '₧');
            Character pot9 = new Character("pot 9", "It's a pot", 120, 1, 0, 0, 0, enemyItem[9], 1, 12, 0, false, '₧');
            Character pot10 = new Character("pot 10", "It's a pot", 120, 1, 0, 0, 0, enemyItem[0], 6, 10, 0, false, '₧');
            Character pot11 = new Character("pot 11", "It's a pot", 120, 1, 0, 0, 0, enemyItem[10], 1, 3, 0, false, '₧');
            Character pot12 = new Character("pot 12", "It's a pot", 120, 1, 0, 0, 0, enemyItem[11], 8, 5, 0, false, '₧');
        ...
        public static string GetString(string prompt, string[] valid, string error)
        {
            ...
            do
            {
                Interface.print(prompt);
                response = Console.ReadLine().ToUpper();
                foreach (string s in valid)
                    if (response == s.ToUpper())
                        OK = true;
                if (!OK)
                {
        ...
        public static bool GetYN(string prompt, string error)
        {
            string[] yn = { "YES", "NO", "Y", "N" };
